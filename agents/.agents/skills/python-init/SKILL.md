...
Guide for initializing Python projects with modern tooling and best practices, based on [trailofbits/cookiecutter-python](https://github.com/trailofbits/cookiecutter-python).
...
## Persist Durable Conventions in the Repo

After initializing or modernizing a repo, create or update the repo's `README.md` and `AGENTS.md` so the conventions survive beyond this one-time setup.

- `README.md`: document human-facing setup and daily commands. Prefer the repo's canonical entrypoints (`make test`, `just test`, `uv run pytest`, etc.) rather than listing every possible raw tool invocation.
- `AGENTS.md`: document agent-facing guardrails and tool preferences. Record durable rules such as using `uv add` / `uv remove` instead of `uv pip install`, using `uv run` instead of manually activating virtual environments, and the canonical lint, test, and typecheck commands.
- Keep only repo-specific policy and entrypoints. Do not copy generic tool comparisons, migration advice, or the whole skill into the repo docs.
...
- [ ] Document canonical setup, dependency, lint, test, and typecheck commands in `README.md` and `AGENTS.md`
